...
class ParticleSystem {
  ...
  init() {
    ...
    if (elements.globalBackground) {
      // Canvas'ı .hero-particles veya .hero-grid'den sonra, ama .global-background'un içinde ekle
      ...
      if (heroParticlesDiv && heroParticlesDiv.nextSibling) {
        ...
        // Fallback eğer .hero-particles yoksa
        elements.globalBackground.appendChild(this.canvas);
      }
    } else {
      document.body.insertBefore(this.canvas, document.body.firstChild);
      console.warn('.global-background elementi bulunamadı, parçacık canvası body\'e eklendi.');
    }
...
const cursorAnimation = {
  ...
  scaleCursor(isHovering, elementType) {
    ...
    let cursorBg = 'var(--primary)'; // Varsayılan cursor rengi

    if (document.documentElement.getAttribute('data-theme') === 'dark') {
      cursorBg = 'var(--primary-light)'; // Karanlık modda cursor rengi
    }
    ...
    if (isHovering) {
      ...
      followerBorder = `2px solid ${cursorBg}`; // Tema uyumlu border
    }

    elements.cursor.style.backgroundColor = cursorBg; // Cursor rengini ayarla
...
const projectFilter = {
  ...
  filterProjects(e) {
    ...
    projects.forEach(project => {
      project.style.transition = 'opacity 0.3s ease, transform 0.3s ease'; // Yumuşak geçiş
...
const formManager = {
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      if (generalErrorElement) generalErrorElement.textContent = 'Mesaj gönderilirken bir hata oluştu. Lütfen tekrar deneyin veya doğrudan e-posta gönderin.';
      else alert('Mesaj gönderilirken bir hata oluştu. Lütfen tekrar deneyin.');
    } finally {
...
const initAOS = () => {
  if (typeof AOS !== 'undefined') {
    ...
    console.warn('AOS kütüphanesi bulunamadı.');
  }
};
...
const skillsAnimation = {
  ...
  observeSkillSections() {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // Dot animasyonu
          setTimeout(() => {
            section.querySelectorAll('.skill-dot.active').forEach((dot, index) => {
              dot.style.opacity = '0';
              setTimeout(() => {
                dot.style.opacity = '1';
              }, index * 150);
          ...
          // Skill item animasyonu
          section.querySelectorAll('.skill-item, .tech-item').forEach((item, index) => {
    ...
    // Tüm skill section'larını gözlemle
  ...
  bindEvents() {
    // Pencere boyutu değiştiğinde animasyonları güncelle
    window.addEventListener('resize', utils.debounce(() => {
      this.observeSkillSections();
    }, 200));
  }
